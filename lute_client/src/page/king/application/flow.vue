<template>
	<div>
		<van-nav-bar class="top" title="流量充值" fixed left-arrow @click-left="onClickLeft" />
		...
		<div class="youkaBox">
			<div style="overflow: hidden;">
				<p :class="item.select ? 'price yes': 'price'" v-for="(item,index) in list" :key="index" @click="QYC.click('choose',index)">{{item.liuliang}}</span></p>
			</div>
			<!-- <div class="zongjia">价格：{{price}}元</div> -->
		</div>
	...
	export default {
		...
		methods: {
			...
			getFlow() {
				...
				self.QYC.get({
					name: 'phone',
					url: process.env.BASE_API + '/market/getliuliangdata',
					params: {
						jwt: self.jwt
					}
				})
			...
			on_click_choose(index) {
				...
				self.rmb = self.list[index].liuliang;
				self.price = self.list[index].money;
			...
			buyConfirm() {
				...
				self.QYC.get({
					name: 'chongzhi',
					url: process.env.BASE_API + '/market/liuliangchongzhi',
					params: {
						jwt: self.jwt,
						rmb: self.price,
						czphone: self.phone,
